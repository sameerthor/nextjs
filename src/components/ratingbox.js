...
export default function RatingBox({store}) {
    ...
    if (process.browser)
        var user_rating = localStorage.getItem(store.store.id + "_rating") || 0
    else
        var user_rating = 0;
...
//console.log("r",user_rating)
    ...
    const handleSubmit = async () => {
        ...
        try {
        //    console.log("store_id", store.store.id)
            const res = await fetch('/api/rating', {
                method: 'POST',
                body: JSON.stringify({ "rating": { "itemId": store.store.id, "values": [userRating] } }),
                headers: {
        ...
        modal.hide();
        setModalOpen(false);
        localStorage.setItem(store.store.id + "_rating", userRating)
        setRateAlready(true)
    }
                        ...
                        <div className="modal-content" >
                            <div className="modal-header" style={{ justifyContent: "space-between" }}>
                                <h5 className="modal-title" id="exampleModalLabel">
                                    {rateAlready == true ? "Thanks For your Rating!!!" : `Rate ${store.store.name}`}
                                </h5>
                                <button
                                    type="button"
